...
public static class StreamingAssetsExtension
{
    ...
    public static List<string> GetFoldersRecursively(string path, ref List<string> paths)
    {
        var fullPath = Path.Combine(Application.streamingAssetsPath, path);
        DirectoryInfo dirInfo = new DirectoryInfo(fullPath);
        foreach (var folder in dirInfo.GetDirectories())
        {
            if (!folder.Name.Contains(".meta"))
            {
                paths.Add(Path.Combine(path, folder.Name)); // With file extension
            }
        }

        foreach (var dir in dirInfo.GetDirectories())
        {
            GetFoldersRecursively(Path.Combine(path, dir.Name), ref paths);
        }
    ...
    public static List<string> GetFoldersRecursively(string path)
    {
        List<string> folders = new List<string>();
        return GetFoldersRecursively(path, ref folders);
    }
}